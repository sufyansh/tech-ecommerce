...
const ProductDetail = () => {
    ...
    return (
        <motion.div
            ...
            exit="exit"
        >
            <PageHero title={product.product.category} product />
            <div className='mt-16 space-y-16 w-[80vw] mx-auto'>
                ...
                {loading ? (
                    ...
                    <div className='grid grid-cols-1 lg:grid-cols-2 gap-16'>
                        ...
                        <div>
                            <h2 className='font-bold text-5xl tracking-wide mb-5'>{product.product.name}</h2>
                            <Stars />
                            <h4 className='text-xl font-extrabold text-secondary-100 tracking-widest italic my-4'>{formatPrice(product.product.price)}</h4>
                            <p className='max-w-3xl tracking-wider leading-8 text-gray-500 mb-6'>{product.product.description}</p>
                            <div className='flex flex-col w-full sm:w-3/4 lg:w-1/2 space-y-5'>
                                ...
                                <div className='flex justify-between'>
                                    <p className='text-lg font-semibold tracking-wider text-gray-600'>Size :</p>
                                    <p>Height: {product.product.dimension}</p>
                                    <p>Width: {product.product.width}</p>
                                </div>
                                <div className='flex justify-between'>
                                    <p className='text-lg font-semibold tracking-wider text-gray-600'>Category :</p>
                                    <p className='uppercase'>{product.product.category}</p>
                                </div>
                            </div>
                            <hr className='my-6' />
